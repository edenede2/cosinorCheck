...
def download_results(results, original_data, window_size, period, select_period_size):
    ...
    st.write(f"select_period_size: {select_period_size}")
    st.write(f"window_size: {window_size}")
    for key in results.keys():
        ...
        if select_period_size > 24:
            trough_days = int((y_estimated_min_loc / length) * (period* window_size)) // select_period_size 
            if '12:00:00' in key:
...
def main():
    ...
    if uploaded_file is not None:
        try:
            ...
            select_period_size = st.slider("Select the period size (in days)",  12, 168, 24, 12)
            st.write(f"Selected period size: {select_period_size / 24} days")
